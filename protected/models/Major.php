...
class Major extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id_country, id_region, id_city, form, weight, cost, price, insurance, delivery_time, order_id, address', 'required'),
			array('id_country, id_region,id_city, weight, order_id', 'numerical', 'integerOnly'=>true),
			array('cost, price, insurance', 'numerical'),
			array('delivery_time', 'length', 'max'=>255),
			array('form', 'length', 'max'=>25),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, id_country, id_region, form, weight, cost, price, insurance, delivery_time, order_id, date_added', 'safe', 'on'=>'search'),
		);
	}
