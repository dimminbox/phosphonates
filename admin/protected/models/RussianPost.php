...
class RussianPost extends CActiveRecord
{
	...
	{
		return Yii::app()->params['table_suffix'].'RussianPost';
	}
	...
	public function rules()
	{
		...
		return array(
			array('country, weight, price, cost, method, method_id, country_id', 'required'),
			array('weight, method_id, country_id', 'numerical', 'integerOnly'=>true),
			array('price, cost', 'numerical'),
			array('country, method', 'length', 'max'=>255),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, country, weight, price, cost, method, method_id, country_id', 'safe', 'on'=>'search'),
		);
	}
