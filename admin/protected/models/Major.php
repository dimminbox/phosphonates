...
class Major extends CActiveRecord
{
	...
	{
		return Yii::app()->params['table_suffix'].'Major';
	}
	...
	public function rules()
	{
		...
		return array(
			array('id_country, country, region_id, region, form, weight, cost, price, insurance, delivery_time, order_id', 'required'),
			array('id_country, region_id, weight, order_id', 'numerical', 'integerOnly'=>true),
			array('cost, price, insurance', 'numerical'),
			array('country, delivery_time', 'length', 'max'=>255),
			array('region', 'length', 'max'=>512),
			array('form', 'length', 'max'=>25),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, id_country, country, region_id, region, form, weight, cost, price, insurance, delivery_time, order_id', 'safe', 'on'=>'search'),
		);
	}
