...
class OfferController extends Controller
{
	...
	{
            $action = Yii::app()->urlManager->createUrl('offer/create');
            $lang = new Language;
            $lang->name = $language;
            ...
            if(isset($_POST['Offer']))
            {
                    ...
                    if ($_FILES['Offer']['name']['image']!=''){
                        ...
                        $upload->saveAs(Yii::app()->params['image_of'].$model->image);
                    }

                    if (($of_lang->validate())&&($model->validate()))
                        if($model->save())
	...
	{
		$action = Yii::app()->urlManager->createUrl('offer/update',array('id'=>$id));
                $id_lang = Language::model()->findByAttributes(array('code'=>$language))->id;
		$model = $this->loadModel($id);
                $of_lang = new OfferLang();
                $_of_lang = OfferLang::model()->findByAttributes(array('id_offer'=>$id,'id_lang'=>$id_lang));
                $of_lang->attributes = (isset($_of_lang->attributes)) ? $_of_lang->attributes : array('id_offer'=>0,'id_lang'=>$id_lang);
